<template>
	<div ref="layout" class="grid w-screen h-screen divide-x">
		<header class="container bg-black">
			...
			<ul class="grid gap-8 justify-center">
				...
				<router-link to="/" :class="'attendance' === activeMenu ? 'text-white' : 'text-white/40'" class="hover:text-white transition-all text-center flex cursor-pointer items-center gap-6 h-6">
					<MailIcon class="w-4" />
					<span v-if="!colapsed">Messages</span>
				</router-link>
				<router-link to="/widgets" :class="'widgets' === activeMenu ? 'text-white' : 'text-white/40'" class="hover:text-white transition-all text-center flex cursor-pointer items-center gap-6 h-6">
					<CogIcon class="w-4" />
					<span v-if="!colapsed">Widgets</span>
				</router-link>
				<li class="text-white/40 hover:text-white transition-all text-center flex cursor-pointer items-center gap-6 h-6">
					<CogIcon class="w-4" />
...
export default {
	...
	methods: {
		...
		changeLayout() {
			let gridCLass = this.activeMenu === 'widgets' ? '83%' : 'minmax(308px,25%) auto'
			let style = !this.colapsed ? `17% ${gridCLass}` : `0fr ${gridCLass}`
			this.$refs.layout.setAttribute('style', `grid-template-columns: ${style};`)
		}
	},
	created() {
		this.$watch(() => this.$route, route => {
